package stock.dzh;

/**
 * Created by Luonanqin on 4/7/15.
 */
public class DzhOtherData {

	private String wb;
	private String wc;
	private String lp; // 当前价
	private String wp;
	private String np;
	private String s5p;
	private String s5v;
	private String s4p;
	private String s4v;
	private String s3p;
	private String s3v;
	private String s2p;
	private String s2v;
	private String s1p;
	private String s1v;
	private String b5p;
	private String b5v;
	private String b4p;
	private String b4v;
	private String b3p;
	private String b3v;
	private String b2p;
	private String b2v;
	private String b1p;
	private String b1v;

	public String getWb() {
		return wb;
	}

	public void setWb(String wb) {
		this.wb = wb;
	}

	public String getWc() {
		return wc;
	}

	public void setWc(String wc) {
		this.wc = wc;
	}

	public String getLp() {
		return lp;
	}

	public void setLp(String lp) {
		this.lp = lp;
	}

	public String getWp() {
		return wp;
	}

	public void setWp(String wp) {
		this.wp = wp;
	}

	public String getNp() {
		return np;
	}

	public void setNp(String np) {
		this.np = np;
	}

	public String getS5p() {
		return s5p;
	}

	public void setS5p(String s5p) {
		this.s5p = s5p;
	}

	public String getS5v() {
		return s5v;
	}

	public void setS5v(String s5v) {
		this.s5v = s5v;
	}

	public String getS4p() {
		return s4p;
	}

	public void setS4p(String s4p) {
		this.s4p = s4p;
	}

	public String getS4v() {
		return s4v;
	}

	public void setS4v(String s4v) {
		this.s4v = s4v;
	}

	public String getS3p() {
		return s3p;
	}

	public void setS3p(String s3p) {
		this.s3p = s3p;
	}

	public String getS3v() {
		return s3v;
	}

	public void setS3v(String s3v) {
		this.s3v = s3v;
	}

	public String getS2p() {
		return s2p;
	}

	public void setS2p(String s2p) {
		this.s2p = s2p;
	}

	public String getS2v() {
		return s2v;
	}

	public void setS2v(String s2v) {
		this.s2v = s2v;
	}

	public String getS1p() {
		return s1p;
	}

	public void setS1p(String s1p) {
		this.s1p = s1p;
	}

	public String getS1v() {
		return s1v;
	}

	public void setS1v(String s1v) {
		this.s1v = s1v;
	}

	public String getB5p() {
		return b5p;
	}

	public void setB5p(String b5p) {
		this.b5p = b5p;
	}

	public String getB5v() {
		return b5v;
	}

	public void setB5v(String b5v) {
		this.b5v = b5v;
	}

	public String getB4p() {
		return b4p;
	}

	public void setB4p(String b4p) {
		this.b4p = b4p;
	}

	public String getB4v() {
		return b4v;
	}

	public void setB4v(String b4v) {
		this.b4v = b4v;
	}

	public String getB3p() {
		return b3p;
	}

	public void setB3p(String b3p) {
		this.b3p = b3p;
	}

	public String getB3v() {
		return b3v;
	}

	public void setB3v(String b3v) {
		this.b3v = b3v;
	}

	public String getB2p() {
		return b2p;
	}

	public void setB2p(String b2p) {
		this.b2p = b2p;
	}

	public String getB2v() {
		return b2v;
	}

	public void setB2v(String b2v) {
		this.b2v = b2v;
	}

	public String getB1p() {
		return b1p;
	}

	public void setB1p(String b1p) {
		this.b1p = b1p;
	}

	public String getB1v() {
		return b1v;
	}

	public void setB1v(String b1v) {
		this.b1v = b1v;
	}
}
