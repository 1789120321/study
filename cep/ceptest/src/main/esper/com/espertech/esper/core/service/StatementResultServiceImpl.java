...
public class StatementResultServiceImpl implements StatementResultService {
	 ...
	 */
	public StatementResultServiceImpl(String statementName, StatementLifecycleSvc statementLifecycleSvc, MetricReportingServiceSPI metricReportingService,
	...
	public void setContext(EPStatementSPI epStatement, EPServiceProviderSPI epServiceProvider, boolean isInsertInto, boolean isPattern, boolean isDistinct,
			boolean isForClause, StatementMetricHandle statementMetricHandle) {
	...
	public void setSelectClause(Class[] selectClauseTypes, String[] selectClauseColumnNames, boolean forClauseDelivery,
			ExprEvaluator[] groupDeliveryExpressions, ExprEvaluatorContext exprEvaluatorContext) {
	...
	public void setUpdateListeners(EPStatementListenerSet statementListenerSet) {
		...
		isMakeSynthetic = !(statementListenerSet.getListeners().isEmpty() && statementListenerSet.getStmtAwareListeners().isEmpty()) || isPattern
				|| isInsertInto || isDistinct | isForClause;
		...
		statementResultNaturalStrategy = ResultDeliveryStrategyFactory.create(statementName, statementListenerSet.getSubscriber(), selectClauseTypes,
	...
	public void indicate(UniformPair<EventBean[]> results) {
		if (results != null) {
			...
			if ((results.getFirst() != null) && (results.getFirst().length != 0)) {
				 ...
				 * 代码优化：
				 * 
				 * 判断EPStatement实例是否为pattern，如果不为pattern，则不保存最新的匹配结果，减少对象引用
				 * 
				 * Code line： 201-204
				 * Author: luonq@primeton.com
				 * Date: 2013-6-16 9:50
				 */
				if (epStatement.isPattern())
				{
					lastIterableEvent = results.getFirst()[0];
				}
