...
public abstract class ExprAggregateNodeBase extends ExprNodeBase implements ExprEvaluator, ExprAggregateNode {
	 ...
	 */
	protected abstract AggregationMethodFactory validateAggregationChild(StreamTypeService streamTypeService, MethodResolutionService methodResolutionService,
	...
	public void validate(ExprValidationContext validationContext) throws ExprValidationException {
		aggregationMethodFactory = validateAggregationChild(validationContext.getStreamTypeService(), validationContext.getMethodResolutionService(),
				validationContext.getExprEvaluatorContext());
	}
	...
	protected final Class validateNumericChildAllowFilter(StreamTypeService streamTypeService, boolean hasFilter) throws ExprValidationException {
		...
		if (!JavaClassHelper.isNumeric(childType)) {
			throw new ExprValidationException("Implicit conversion from datatype '" + childType.getSimpleName()
	...
	public void validateFilter(ExprEvaluator filterEvaluator) throws ExprValidationException {
		if (JavaClassHelper.getBoxedType(filterEvaluator.getType()) != Boolean.class) {
			throw new ExprValidationException("Invalid filter expression parameter to the aggregation function '" + getAggregationFunctionName()
